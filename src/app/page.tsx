...
function FeaturedProducts() {
    ...
    const filteredProducts = featuredProducts
        ?.filter(p => p.name.toLowerCase() !== 'qewqw')
        .slice(0, 3) || [];
          ...
          {filteredProducts.map((product) => {
            ...
            return (
              <Link href={`/products/${product.slug}`} key={product.id} className="group block">
                <div className="relative aspect-[3/4] w-full overflow-hidden rounded-lg border bg-card">
                  {imageUrl ? (
                    <Image
                      src={imageUrl}
                      alt={product.name}
                  ...
                  <div className="absolute inset-0 bg-black/10 group-hover:bg-black/5 transition-colors" />
                   <div className="absolute bottom-4 left-4 right-4 bg-white/90 dark:bg-card/90 backdrop-blur-sm p-3 rounded-md shadow-sm">
                        <h3 className="font-headline text-lg font-bold text-foreground line-clamp-1">{product.name}</h3>
...
export default function Home() {
  ...
  return (
    <div className="space-y-12 md:space-y-20 pb-20">
      ...
      <section id="featured" className="container mx-auto px-4 scroll-mt-20">
      ...
      <section className="container mx-auto px-4">
        <Link href="#featured" className="block relative group overflow-hidden rounded-2xl shadow-2xl h-[400px] md:h-[500px]">
          <Image
